...
interface ApplicationTableProps {
  ...
  toggleEstadoApp: (applicationId: number, currentState: boolean) => void;
  loading: boolean;
}

export const ApplicationTable = ({ data, onEdit, onDelete, toggleEstadoApp, loading }: ApplicationTableProps) => {
  return (
            ...
            {!loading && data.length === 0 ? (
              ...
              data.map((app) => (
                <tr key={app.applicationId}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm dark:text-white">{app.name}</td>
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">{app.description}</td>
                  <td className="px-6 py-4 text-sm text-gray-500">{app.url}</td>
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={app.isActive || false}
                        onChange={() => toggleEstadoApp(app.applicationId, app.isActive || false)}
