import type { Module } from "../interface/modularidadInterface";

interface ModuleTableProps {
    data: Module[]; 
    onEdit: (module: Module) => void; 
    onDelete: (moduleId: number) => void; 
    toggleEstadoModule: (moduleId: number, currentState: boolean) => void;
    loading: boolean;
}

export const ModuleTable = ({ data, onEdit, onDelete, toggleEstadoModule, loading }: ModuleTableProps) => {
    return (
                ...
                {!loading && data.length === 0 ? (
                  ...
                  data.map((module) => (
                    <tr key={module.moduleId}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm dark:text-white">{module.name}</td>
                        <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">{module.application?.name}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">{module.description}</td>
                      <td className="px-6 py-4 text-sm text-gray-500">{module.route}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={module.isActive || false}
                            onChange={() => toggleEstadoModule(module.moduleId, module.isActive || false)}
