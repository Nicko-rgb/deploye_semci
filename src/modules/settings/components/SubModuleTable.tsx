...
interface SubModuleTableProps {
    data: SubModule[]; 
    onEdit: (item: SubModule) => void;
    onDelete: (id: number) => void;
    toggleEstadoSubModule: (id: number, currentState: boolean) => void;
    loading: boolean;
}

export const SubModuleTable = ({ data, onEdit, onDelete, toggleEstadoSubModule, loading }: SubModuleTableProps) => {
    return (
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase whitespace-nowrap w-32">Acciones</th>
                </tr>
              </thead>
                ...
                {!loading && data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-10 text-center text-gray-500">
                      No se encontraron submódulos disponibles.
                    </td>
                  </tr>
                ) : (
                  data.map((subModule) => (
                    <tr key={subModule.submoduleId}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm dark:text-white">{subModule.name}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">{subModule.description}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">{subModule.module?.application?.name}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">{subModule.module?.name}</td>
                      <td className="px-6 py-4 text-sm text-gray-500">{subModule.route}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300">
                        <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={subModule.isActive || false}
                          onChange={() => toggleEstadoSubModule(subModule.submoduleId, subModule.isActive || false)}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap items-center justify-center gap-2 w-28 mx-auto">
                          {/* Botón Editar */}
                          <button 
                          ...
                          {/* Botón Eliminar */}
                          <button 
                            onClick={() => onDelete(subModule.submoduleId)}
